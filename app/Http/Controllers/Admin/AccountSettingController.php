...
class AccountSettingController extends Controller
{
                        ...
                        )
    {
        ...
        $this->module_title                  = "Edit Profile";
    ...
    */
    ...
    public function update(Request $request)
    {
        ...
        $is_admin_avail = $this->UsersModel->where('id',$admin_id)->count();
        ...
        if($is_admin_avail > 0)
        {
            $obj_data = $this->UsersModel->where('user_type', 'admin')->where('id',$admin_id)->update($arr_data);
        }
        else
        {
        ...
        if(Hash::check($current_password,$admin_details->password))
        {
            if($current_password!=$new_password)
            {
                if($new_password == $confirm_password)
                {
                    ...
                    $is_password_changed = $this->UsersModel->where('user_type', 'admin')->where('id',$admin_id)->update(['password'=>$user_password]);
