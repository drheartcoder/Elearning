...
class TextbookController extends Controller
{
    ...
    public function TextbookList(Request $request, $enc_program_id = false)
    {
        ...
        $subject_id   = $grade_id = '';
        $reset_link   = url('/').'/student/textbook';
        $user_id      = Auth::user()->id;

        $getClasses = getStudentAssginClasses($user_id);
        ...
        if( count($getClasses) > 0 && !empty($getClasses) ) {
            ...
            $obj_textbook = $obj_textbook->whereHas("textbookData", function($query) use($arr_class_subject, $arr_class_grade) {
        ...
        if($enc_program_id != '' && !empty($enc_program_id))
        {
            ...
            $reset_link = url('/').'/student/textbook/'.$enc_program_id;
        }
        ...
        $data['middleContent']                 = 'student.textbook.index';
