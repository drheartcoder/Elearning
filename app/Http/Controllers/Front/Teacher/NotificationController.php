...
class NotificationController extends Controller
{
    ...
    */
    public function Index()
    {
        ...
        $obj_notifications = $this->NotificationsModel->where('to_user_id', $teacher_id)->orderBy('id', 'DESC')->paginate(10);
        if($obj_notifications)
        {
            $arr_notifications = $obj_notifications->toArray();
            $arr_pagination    = $obj_notifications->links();
            $update_is_raed    = $this->NotificationsModel->where('to_user_id', $teacher_id)->where('is_read', '0')->update(['is_read'=>'1']);
        }
        //dd($arr_notifications['data']);

        $data['arr_notifications'] = $arr_notifications;
        $data['arr_pagination']    = $arr_pagination;
        $data['pageTitle']         = trans('home.Notifications');
        $data['user_type']         = 'teacher';
        $data['parentTitle']       = trans('parent.Dashboard');
        $data['middleContent']     = 'teacher.notification.index';
    ...
    */
    ...
    public function Delete(Request $request)
    {
        ...
        if($notification_id)
        {
            ...
            $del_notifications = $this->NotificationsModel->where('to_user_id', $teacher_id)->where('id', $id)->delete();
