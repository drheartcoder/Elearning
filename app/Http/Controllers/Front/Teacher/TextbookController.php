...
class TextbookController extends Controller
{
    ...
    public function TextbookList(Request $request, $enc_class_id = false, $enc_program_id = false)
    {
        $arr_textbook = $arr_pagination = $arr_subject = $arr_grade = $arr_class_subject = $arr_class_grade = [];
        $subject_id   = $grade_id = '';
        $reset_link   = url('/').'/teacher/textbook/'.$enc_class_id;
        $user_id      = Auth::user()->id;

        $getClasses = getTeachersAddedClasses($user_id);
        if( count($getClasses) > 0 && !empty($getClasses) ) 
        {
            foreach ($getClasses as $key => $classes) 
            {
                $arr_class_subject[] = $classes['subject_id'];
                $arr_class_grade[]   = $classes['grade_id'];
            }
        }
        ...
        $obj_textbook = $this->TextbookimagesModel->with(['textbookData' => function($query) {
                                                ...
                                                ->whereHas("textbookData", function($query) use($arr_class_subject, $arr_class_grade) {
                                                    $query->whereIn('subject_id', $arr_class_subject);
                                                    $query->whereIn('grade_id', $arr_class_grade);
                                                });
        ...
        if($enc_program_id != '' && !empty($enc_program_id))
        {
            ...
            $reset_link = url('/').'/teacher/textbook/'.$enc_class_id.'/'.$enc_program_id;
        }
        ...
        $data['middleContent']                 = 'teacher.textbook.index';
