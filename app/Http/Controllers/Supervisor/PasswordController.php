...
class PasswordController extends Controller
{
    ...
    function __construct(UsersModel $user_model, HasherContract $hasher)
    {
        ...
        $this->module_url_path       = url('/supervisor');
        ...
        $this->supervisor_id         = isset($this->auth->user()->id)? $this->auth->user()->id:15;
        $this->UsersModel            = $user_model;

        Config::set("auth.defaults.passwords","supervisor");
    }
    ...
    public function postEmail(Request $request)
    {
        ...
        Config::set("auth.user_mode","supervisor"); //sets user mode for sending email
    ...
    public function get_email($token)
    {
        ...
        if($password_reset != NULL)
        {
            ...
            return view('supervisor.auth.reset_password',$this->arr_view_data);    
        }
        else
        {
            return redirect($this->module_url_path)->with('error', 'Your reset password link has been expired.');
