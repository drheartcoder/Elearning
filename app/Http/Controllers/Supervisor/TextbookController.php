...
class TextbookController extends Controller
{
                                ...
                                )
	{
        ...
        $this->module_view_folder            = "supervisor.program.textbook";
        $this->admin_url_path                = url(config('app.project.admin_panel_slug'));
        $this->admin_panel_slug              = config('app.project.admin_panel_slug');
        $this->module_url_path               = url(config('app.project.supervisor_panel_slug')."/program/material/");
        $this->program_module_url_path       = url(config('app.project.supervisor_panel_slug')."/program");
    ...
    public function update($program_id = '',$enc_id = '',Request $request)
    {
        ...
        $isNameExist = $this->TextbookModel->where('id', '<>', $textbookId)->where('name', '=', $request->input('textbookName'))->count();
        if($isNameExist > 0)
        {
