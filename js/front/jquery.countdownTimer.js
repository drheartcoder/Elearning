...
(function($) {
    var methods = {
        ...
        clearTime: function(options) {
            var opts = $.extend({}, $.fn.countdowntimer.defaults, options);
            timerType = "MS";
            delete window['hours_' + timerType + $(this).attr('id')];
            delete window['minutes_' + timerType + $(this).attr('id')];
            delete window['seconds_' + timerType + $(this).attr('id')];
            clearInterval(window['hours_' + timerType + $(this).attr('id')]);
            clearInterval(window['minutes_' + timerType + $(this).attr('id')]);
            clearInterval(window['seconds_' + timerType + $(this).attr('id')]);
    ...
    function minutesSeconds($this, opts) {
        var id = $this.attr('id');
        var time = "";
        if((window['minutes_MS' + id]!=null && window['minutes_MS' + id]!=undefined) && (window['seconds_MS' + id]!=null && window['seconds_MS' + id]!=undefined))
        {
            if (window['minutes_MS' + id] === opts.minutes && window['seconds_MS' + id] === opts.seconds) {
                ...
                if(is_answer=="no")
                {
                    ...
                    var remaining_minutes = window['minutes_MS' + id]!=undefined ? window['minutes_MS' + id] : 0;
                    var remaining_seconds = window['seconds_MS' + id]!=undefined ? window['seconds_MS' + id] : 0;
                    $('#remaining_minutes').val(remaining_minutes);
                    $('#remaining_seconds').val(remaining_seconds);
                    sessionStorage.setItem('remaining_seconds_'+storage_id,remaining_seconds);
                    sessionStorage.setItem('remaining_minutes_'+storage_id,remaining_minutes);
                }
                else
                {
                    sessionStorage.setItem('remaining_minutes_'+storage_id,0);
                    sessionStorage.setItem('remaining_seconds_'+storage_id,0);
    ...
    function clearTimerInterval($this, timerType, opts) {
        ...
        if (($this.data('countdowntimer')!=null && $this.data('countdowntimer')!=undefined) && $this.data('countdowntimer').destroy === true) {
