...
<!--                        <label class="bmd-label-floating">User Type </label>-->
                      <div class="form-group has-default bmd-form-group is-filled">
                           <select class="selectpicker" data-rule-required="true" data-style="select-with-transition" name="user_type" id="user_type">
                              ...
                              <option value="program-creator" @if(isset($user_type) && $user_type!='' && $user_type=='program-creator') selected @endif>Program Creator</option>
                              <option value="supervisor" @if(isset($user_type) && $user_type!='' && $user_type=='supervisor') selected @endif>Supervisor</option>
                              <option value="subadmin" @if(isset($user_type) && $user_type!='' && $user_type=='subadmin') selected @endif>Subadmin</option>                              
                           </select>
                        </div>
                    </div>
                    <div class="col-md-4">
...
$(document).ready(function() {
	...
	$('#datatables').DataTable({
		...
		{
			'url'  : temp_url,
			'data' : { 'keyword':keyword,'user_type':user_type }
		},
		"columnDefs" : [
