          ...
          <form method="post" name="frmUpdate" id="frmUpdate" action="{{ $base_module_url_path.'/update/'.base64_encode($programId).'/'.base64_encode($textbookId) }}" class="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="card-body">
              @include('admin.layout._operation_status')
                  ...
                  {
                    if(isset($arrMaterial['homework_images_data']))
                    {
                      if(count($arrMaterial['homework_images_data']) > 0)
                      {
                        ?>
                        <div class="row">
                        <?php
                        /*dd($arrMaterial['homework_images_data']);*/
                        foreach ($arrMaterial['homework_images_data'] as $arrMaterialVal) {
                          if(isset($arrMaterialVal['file']) && $arrMaterialVal['file']!='')
                          {
                            if(file_exists($homework_file_base_img_path.$arrMaterialVal['file']))
                            {
                              $fileUrl = $homework_file_public_img_path.$arrMaterialVal['file'];
