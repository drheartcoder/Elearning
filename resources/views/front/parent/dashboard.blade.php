...
<!-- bredcrum section end -->
            ...
            <div class="row">
                ...
                <div class="col-sm-8 col-md-9 col-lg-9">
                    ...
                    <div class="student-report parent-dash">
                                     ...
                                     <div class="form-group">
                                            <div class="name-field">
                                              @if(isset($arr_student) && sizeof($arr_student)>0)
                                                <select name="selected_student" onchange="getStudentPerformance(this.value)" id="selected_student">
                                                    <option value="">{{trans('parent.Select_Student')}}</option>
                                                  @foreach($arr_student as $value)
                                                    @if(isset($value['student_id']) && $value['student_id']!="")
                                                    <option @if(isset($student_id) && $student_id==$value['student_id']) selected="" @endif value="{{ $value['student_id'] or '' }}">{{ $value['user_data']['first_name'] or '' }} {{ $value['user_data']['last_name'] or '' }}</option>
                                                    @endif
                                                  @endforeach
                                                </select>
                                              @endif
                        ...
                        <div class="report-blocks-section parent-dash">
    ...
    function getSelectedType(type)
    {
        var type = $('#type').val();
        var selected_year    = $('#selected_year').val();
        var selected_student = $('#selected_student').val();
        location.href= '{{ $module_url_path }}/'+type+'/'+selected_year+'/'+btoa(selected_student);
    ...
    function getperformanceByYear(year)
    {
        ...
        {
           $('#type_div').show();
           location.href= '{{ $module_url_path }}/'+type+'/'+year;
        }
        else
        {
          $('#type_div').hide();
          location.href= '{{ $module_url_path }}/'+'yearly'+'/'+year;
        }
    }
    function getStudentPerformance(student_id)
    {
         ...
         var selected_year    = '{{ $current_year }}';
          location.href= '{{ $module_url_path }}/'+type+'/'+selected_year+'/'+btoa(student_id);
    }
    </script>