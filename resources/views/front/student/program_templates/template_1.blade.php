...
<div class="middle-section">
        ...
        <div class="fill-in-the-blank-section">
            <div class="question-section">
                <div class="question-txt-section">
                    <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template1']['question']) && $arr_question['template1']['question']!='' ? ucwords($arr_question['template1']['question']) : "N/A" }}
                </div>
            ...
            <div class="row">
                <div class="col-sm-6 col-md-6 col-lg-6">
                    ...
                    <div class="game-img-section  img-height-width-set">
                        @php $result = get_image_public_path($arr_question['template1']['file'],$arr_question['template1']['file_type']);  @endphp
                        @if(isset($arr_question['template1']['file_type']) && $arr_question['template1']['file_type']=='image')
                ...
                @php
                    $arr_word = str_split($arr_question['template1']['question_text'], 1);
                @endphp
                            ...
                            <div class="alphabate-input" style="width: 100%;">
                                @foreach($arr_word as $key => $val)
                                    @if($arr_question['is_answer']=='yes')
                                        ...
                                        <!-- <input class="correct-ans" value="{{$val}}" disabled="" data-answer={{$val}} type="text" name="answer" style="width: 9%" maxlength="1" /> -->
                                    @else
            ...
            <div class="col-sm-6 col-md-4 col-lg-3">
                <div class="specker-mic-section">
                    <button onclick="togglePlay()" id="hornButton">
                        ...
                        <audio id="horn" src="{{$question_audio_public_path.$arr_question['template1']['horn']}}"></audio>
                    </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template1']['duration']));
            $minutes     = date('i',strtotime($arr_question['template1']['duration']));
            $seconds     = date('s',strtotime($arr_question['template1']['duration']));
        }
        $actual_time = $arr_question['template1']['duration'];
