...
@php
    $uniqid = uniqid();
    $answer = $arr_question['template10']['answer'];
    $answer_text = isset($answer) && $answer!=0 ? $arr_question['template10']['option'.$answer] : 'N/A';
    if($arr_question['is_answer']=='yes'){
        $replace    = '<span class="input-txt-block correct-answer-section" id="correctAnswer">'.$answer_text.'</span> <span>';
    }
    else{
        $replace    = '<span class="input-txt-block" id="correctAnswer"> </span> <span>';
    }
    $question       = str_replace('#BLANK#',$replace,$arr_question['template10']['question']);
@endphp
    <!--middle section-->
            ...
            <div class="fill-in-the-blank-section">
                <div class="question-section">
                    <div class="question-txt-section">
                        <i class="fa fa-angle-double-right"></i>{{isset($arr_question['template10']['question']) && $arr_question['template10']['question']!='' ? ucwords($arr_question['template10']['question']) : "N/A" }}
                    </div>
                ...
                <div class="row">
                    <div class="col-sm-6 col-mg-6 col-lg-6">
                        ...
                        <div class="game-img-section">
                            @if(isset($arr_question['template10']['question_file']) && $arr_question['template10']['question_file']!='')
                                @php $result = get_image_public_path($arr_question['template10']['question_file'],'image');  @endphp
                                ...
                                <div class="radio-btns">  
                                    <div class="radio-btn">
                                        @if(isset($arr_question['is_answer']) && $arr_question['is_answer']=='yes')
                                            <input type="radio" id="f-option" name="template10_checkbox" data-option="{{$arr_question['template10']['option1']}}" value="1" disabled="true" @if($arr_question['template10']['answer']==1) checked="true" @endif>
                                        @else
                                            <input type="radio" id="f-option" name="template10_checkbox" data-option="{{$arr_question['template10']['option1']}}" value="1" onclick="checkFinalAnswer(this)">
                                        @endif
                                        <label for="f-option">{{isset($arr_question['template10']['option1']) && $arr_question['template10']['option1']!='' ? $arr_question['template10']['option1'] : "N/A"}}</label>                                        
                                    </div>
                                    <div class="radio-btn">
                                        @if(isset($arr_question['is_answer']) && $arr_question['is_answer']=='yes')
                                            <input type="radio" id="s-option" name="template10_checkbox" data-option="{{$arr_question['template10']['option2']}}" value="2" disabled="true" @if($arr_question['template10']['answer']==2) checked="true" @endif>
                                        @else
                                            <input type="radio" id="s-option" name="template10_checkbox" data-option="{{$arr_question['template10']['option2']}}" value="2" onclick="checkFinalAnswer(this)">
                                        @endif
                                        <label for="s-option">{{isset($arr_question['template10']['option2']) && $arr_question['template10']['option2']!='' ? $arr_question['template10']['option2'] : "N/A"}}</label>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-sm-6 col-md-4 col-lg-3">
                    <div class="specker-mic-section">
                        <button onclick="togglePlay()" id="hornButton">
                            ...
                            <audio id="horn" src="{{$question_audio_public_path.$arr_question['template10']['horn']}}"></audio>
                        </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template10']['duration']));
            $minutes     = date('i',strtotime($arr_question['template10']['duration']));
            $seconds     = date('s',strtotime($arr_question['template10']['duration']));
        }
        $actual_time = $arr_question['template10']['duration'];
...
<script type="text/javascript">
    ...
    var correct_answer                = "{{$arr_question['template10']['answer']}}";
</script>
...
<script type="text/javascript">
    ...
    function checkFinalAnswer(ref)
    {
        ...
        $('#correctAnswer').removeClass('wrong-answer-section');
        $(ref).parent().removeClass('wrong-answer-section');
        $('#correctAnswer').removeClass('correct-answer-section');

        $(ref).parent().removeClass('time-over-answer-section');
        if($(ref).val()== correct_answer)
        {
            $('#correctAnswer').html($(ref).data('option'))
            $('#correctAnswer').addClass('correct-answer-section');
        ...
        {
            $('#correctAnswer').html($(ref).data('option'))
            /*$('#correctAnswer').addClass('wrong-answer-section');*/
            $(ref).parent().addClass('wrong-answer-section');
    ...
    {
        $('input[name="template10_checkbox"]').each(function(){
