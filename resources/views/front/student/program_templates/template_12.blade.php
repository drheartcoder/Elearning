...
@php
    $uniqid = uniqid();
    $maxlength = strlen($arr_question['template12']['answer2']);
@endphp
    <!--middle section-->
            ...
            <div class="fill-in-the-blank-section">
                <div class="question-section">
                    <div class="question-txt-section">
                        <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template12']['question']) && $arr_question['template12']['question']!='' ? ucwords($arr_question['template12']['question']) : "N/A" }}
                    </div>
                ...
                <div class="row">
                    <div class="col-sm-6 col-mg-6 col-lg-6">
                        ...
                        <div class="game-img-section">
                            @if(isset($arr_question['template12']['question_1_file']) && $arr_question['template12']['question_1_file']!='')
                                @php $result = get_image_public_path($arr_question['template12']['question_1_file'],'image');  @endphp
                                <img src="{{$result['image_url']}}" alt="tamplate 12" />
                            @endif
                            <div class="ques-ans-section-block">
                                {{$arr_question['template12']['answer1']}}
                                <!-- @if($arr_question['is_answer']=='yes')
                                    <input class="correct-ans uppercase-style" type="text" name="answer1" maxlength="{{$maxlength}}" disabled="true" value="{{$arr_question['template12']['answer1']}}" data-answer="{{$arr_question['template12']['answer1']}}"/>
                                @else
                                    <input class="actual-ans questionCls uppercase-style" type="text" name="answer1" maxlength="{{$maxlength}}" onkeyup="checkAnswer(this,event)" data-answer="{{$arr_question['template12']['answer1']}}"/>
                                @endif -->
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6 col-mg-6 col-lg-6">
                        <div class="game-fill-text-section">
                            @if(isset($arr_question['template12']['question_2_file']) && $arr_question['template12']['question_2_file']!='')
                                @php $result = get_image_public_path($arr_question['template12']['question_2_file'],'image');  @endphp
                                <img src="{{$result['image_url']}}" alt="tamplate 12" />
                            @endif
                            ...
                            <div class="ques-ans-section-block">
                                @if($arr_question['is_answer']=='yes')
                                    <input class="correct-ans uppercase-style" type="text" name="answer2" maxlength="{{$maxlength}}" disabled="true" value="{{$arr_question['template12']['answer2']}}" data-answer="{{$arr_question['template12']['answer2']}}" />
                                @else
                                    <input class="actual-ans questionCls uppercase-style" type="text" name="answer2" maxlength="{{$maxlength}}" onkeyup="checkAnswer(this,event)" data-answer="{{$arr_question['template12']['answer2']}}"/>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-sm-6 col-md-4 col-lg-3">
                    <div class="specker-mic-section">
                        <button onclick="togglePlay()" id="hornButton">
                            ...
                            <audio id="horn" src="{{$question_audio_public_path.$arr_question['template12']['horn']}}"></audio>
                        </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template12']['duration']));
            $minutes     = date('i',strtotime($arr_question['template12']['duration']));
            $seconds     = date('s',strtotime($arr_question['template12']['duration']));
        }
        $actual_time = $arr_question['template12']['duration'];
...
<script type="text/javascript">
    ...
    {
        $('.questionCls').each(function(ans_index,ans_value){
