...
<div class="middle-section temp-15-main-section">
        ...
        <div class="fill-in-the-blank-section">
            <div class="question-section">
                <div class="question-txt-section">
                    <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template15']['question']) && $arr_question['template15']['question']!='' ? ucwords($arr_question['template15']['question']) : "N/A" }}
                </div>
            ...
            <div class="row">
                    <div class="col-sm-6 col-mg-6 col-lg-6">
                        ...
                        @endif
                        @php
                        $arr_word1     = str_split($arr_question['template15']['question_1_text'], 1);
                        $arr_position1 = str_split($arr_question['template15']['question_1_answer_position'], 1);
                        $arr_word2     = str_split($arr_question['template15']['question_2_text'], 1);
                        $arr_position2 = str_split($arr_question['template15']['question_2_answer_position'], 1);
                    @endphp
                        <div class="game-img-section">
                            @if(isset($arr_question['template15']['question_1_file']) && $arr_question['template15']['question_1_file']!='')
                                @php $result = get_image_public_path($arr_question['template15']['question_1_file'],'image');  @endphp
                                        ...
                                        @if($word_key==$position_key)
                                            @if($arr_question['is_answer']=='yes')
                                                @if($position_val==1)
                                                    <span>{{$val}}</span>
                                                @else
                                                    <input class="correct-ans" type="text" value="{{$val}}" disabled="true" data-answer={{$val}} name="answer" maxlength="1">
                                                @endif
                                            @else
                                                @if($position_val==1)
                                                    <span>{{$val}}</span>
                                                @else
                                                    <input class="actual-ans questionCls" onkeyup="checkAnswer(this,event)" value="" data-answer={{$val}} type="text" name="image name" maxlength="1" />
                                                @endif
                                            @endif
                                        @endif
                                    @endforeach
                    ...
                    <div class="col-sm-6 col-mg-6 col-lg-6">
                        <div class="game-fill-text-section">
                            @if(isset($arr_question['template15']['question_2_file']) && $arr_question['template15']['question_2_file']!='')
                                @php $result = get_image_public_path($arr_question['template15']['question_2_file'],'image');  @endphp
                                <img src="{{$result['image_url']}}" alt="tamplate 11" />
                            @endif
            ...
            <div class="col-sm-6 col-md-4 col-lg-3">
                <div class="specker-mic-section">
                    <button onclick="togglePlay()" id="hornButton">
                        ...
                        <audio id="horn" src="{{$question_audio_public_path.$arr_question['template15']['horn']}}"></audio>
                    </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template15']['duration']));
            $minutes     = date('i',strtotime($arr_question['template15']['duration']));
            $seconds     = date('s',strtotime($arr_question['template15']['duration']));
        }
        $actual_time = $arr_question['template15']['duration'];
