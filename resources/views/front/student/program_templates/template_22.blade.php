...
@php
    $uniqid = uniqid();
    $str_options = $arr_question['template22']['answer_1'].','.$arr_question['template22']['answer_2'];
...
@endif
@php
    $answer_text1 = isset($arr_question['template22']['answer_1']) && $arr_question['template22']['answer_1']!='' ? $arr_question['template22']['answer_1'] : 'N/A';
    $answer_text2 = isset($arr_question['template22']['answer_2']) && $arr_question['template22']['answer_2']!='' ? $arr_question['template22']['answer_2'] : 'N/A';

    if($arr_question['is_answer']=='yes'){
        $replace1    = '<input class="correct-ans" type="text" name="question1" value="'.$answer_text1.'" disabled="true" />';
        $replace2    = '<input class="correct-ans" type="text" name="question2" value="'.$answer_text2.'" disabled="true" />';
    }
    else{
        $replace1    = '<input class="actual-ans questionCls" type="text" name="question1" maxlength="'.$maxlength.'" data-answer="'.$answer_text1.'" onkeyup="checkAnswer(this,event)" />';
        $replace2    = '<input class="actual-ans questionCls" type="text" name="question2" maxlength="'.$maxlength.'" data-answer="'.$answer_text2.'" onkeyup="checkAnswer(this,event)" />';
    }
    $question1       = str_replace('#BLANK#',$replace1,$arr_question['template22']['question_1']);
    $question2       = str_replace('#BLANK#',$replace2,$arr_question['template22']['question_2']);
@endphp
    <!--middle section-->
            ...
            <div class="fill-in-the-blank-section">
                <div class="question-section">
                    <div class="question-txt-section">
                        <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template22']['question']) && $arr_question['template22']['question']!='' ? ucwords($arr_question['template22']['question'])." : " : "N/A" }} <br>
                    </div>
                ...
                <div class="col-sm-6 col-md-4 col-lg-3">
                    <div class="specker-mic-section">
                        <button onclick="togglePlay()" id="hornButton">
                            ...
                            <audio id="horn" src="{{$question_audio_public_path.$arr_question['template22']['horn']}}"></audio>
                        </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template22']['duration']));
            $minutes     = date('i',strtotime($arr_question['template22']['duration']));
            $seconds     = date('s',strtotime($arr_question['template22']['duration']));
        }
        $actual_time = $arr_question['template22']['duration'];
