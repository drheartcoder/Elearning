...
@php
    $uniqid = uniqid();
    $str_options = $arr_question['template31']['answer'];
    $arr_options = explode(',',$str_options);
    $question_string = $arr_question['template31']['question_1'];
    $maxlength   = 0;
    $question    = '';
@endphp
@if(isset($arr_options) && count($arr_options)>0)
    @foreach($arr_options as $key => $val)
        ...
        @if($arr_question['is_answer']=='yes')
            @php
            $replace    = '<input class="correct-ans" type="text" name="question" value="'.strtolower($val).'" disabled="true" />';
            @endphp
        @else
            @php
            $replace    = '<input class="actual-ans questionCls" type="text" name="question" maxlength="'.$maxlength.'" data-answer="'.strtolower($val).'" onkeyup="checkAnswer(this,event)" />';
            @endphp
        @endif
        @php
         $question_string = preg_replace('/#BLANK#/', $replace, $question_string,1);
...
@endif
    ...
    <!--middle section-->
    <div class="middle-section temp-31-main-section">
        <div class="container">
            <div class="fill-in-the-blank-section">
                <div class="question-section">
                    <div class="question-txt-section">
                        <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template31']['question']) && $arr_question['template31']['question']!='' ? ucwords($arr_question['template31']['question'])." : " : "N/A" }} <br>
                    </div>
                ...
                <div class="col-sm-6 col-md-4 col-lg-3">
                    <div class="specker-mic-section">
                        <button onclick="togglePlay()" id="hornButton">
                            ...
                            <audio id="horn" src="{{$question_audio_public_path.$arr_question['template31']['horn']}}"></audio>
                        </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template31']['duration']));
            $minutes     = date('i',strtotime($arr_question['template31']['duration']));
            $seconds     = date('s',strtotime($arr_question['template31']['duration']));
        }
        $actual_time = $arr_question['template31']['duration'];
