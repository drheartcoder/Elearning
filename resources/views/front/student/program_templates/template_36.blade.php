...
<div class="middle-section temp-36-main-section">
        ...
        <div class="fill-in-the-blank-section">
            <div class="question-section">
                <div class="question-txt-section">
                    <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template36']['question']) && $arr_question['template36']['question']!='' ? ucwords($arr_question['template36']['question']) : "N/A" }}
                </div>
                ...
                <div class="col-sm-6 col-mg-6 col-lg-6">
                    ...
                    <div class="game-img-section">
                       <div class="temp-36-img">
                            @if(isset($arr_question['template36']['question_1_file']) && $arr_question['template36']['question_1_file']!='')
                            @php $result = get_image_public_path($arr_question['template36']['question_1_file'],'image');  @endphp
                            <img src="{{$result['image_url']}}" alt="tamplate 12" />
                            @endif                           
                            @if(isset($arr_question['template36']['question_2_file']) && $arr_question['template36']['question_2_file']!='')
                            @php $result = get_image_public_path($arr_question['template36']['question_2_file'],'image');  @endphp
                            <img src="{{$result['image_url']}}" alt="tamplate 12" />
                            @endif
                       </div>
                    </div>
                    ...
                    <div class="game-fill-text-section">
                        <div class="fill-txt-section">
                           @if($arr_question['is_answer']=='yes')
                            <input class="input-number correct-ans" type="text" style="text-align: center;" value="{{$arr_question['template36']['answer']}}" disabled="true" data-answer="{{$arr_question['template36']['answer']}}" name="question1" maxlength="{{strlen($arr_question['template36']['answer'])}}">
                          @else
                            <input class="input-number actual-ans questionCls" style="text-align: center;" onkeyup="checkAnswer(this,event)" value="" data-answer="{{$arr_question['template36']['answer']}}" type="text" name="question1" maxlength="{{strlen($arr_question['template36']['answer'])}}" />
                          @endif
                        </div>
            ...
            <div class="col-sm-6 col-md-4 col-lg-3">
                <div class="specker-mic-section" style="display: none">
                    <button onclick="togglePlay()" id="hornButton">
                        ...
                        <audio id="horn" src="{{$question_audio_public_path.$arr_question['template36']['horn']}}"></audio>
                    </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template36']['duration']));
            $minutes     = date('i',strtotime($arr_question['template36']['duration']));
            $seconds     = date('s',strtotime($arr_question['template36']['duration']));
        }
        $actual_time = $arr_question['template36']['duration'];
