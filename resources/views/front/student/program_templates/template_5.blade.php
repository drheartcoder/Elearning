...
@endphp
            ...
            <div class="fill-in-the-blank-section">
                <div class="question-section">
                    <div class="question-txt-section">
                        <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template5']['question']) && $arr_question['template5']['question']!='' ? ucwords($arr_question['template5']['question']) : "N/A" }}
                    </div>
                ...
                <div class="row">
                    <div class="col-sm-6 col-mg-6 col-lg-6">
                        ...
                        <div class="game-img-section">
                            @php $result = get_image_public_path($arr_question['template5']['file'],$arr_question['template5']['file_type']);  @endphp
                            @if(isset($arr_question['template5']['file']) && $arr_question['template5']['file_type']=='image')
                                ...
                                <div class="radio-btns">  
                                    <div class="radio-btn">
                                        @if(isset($arr_question['is_answer']) && $arr_question['is_answer']=='yes')
                                            <input type="radio" id="f-option" name="template5_checkbox" value="1" disabled="true" @if($arr_question['template5']['answer']==1) checked="true" @endif>
                                        @else
                                            <input type="radio" id="f-option" name="template5_checkbox" value="1" onclick="checkFinalAnswer(this)">
                                        @endif
                                        <label for="f-option">{{isset($arr_question['template5']['option1']) && $arr_question['template5']['option1']!='' ? $arr_question['template5']['option1'] : "N/A"}}</label>
                                    </div>
                                    <div class="radio-btn">
                                        @if(isset($arr_question['is_answer']) && $arr_question['is_answer']=='yes')
                                            <input type="radio" id="s-option" name="template5_checkbox" value="2" disabled="true" @if($arr_question['template5']['answer']==2) checked="true" @endif>
                                        @else
                                            <input type="radio" id="s-option" name="template5_checkbox" value="2" onclick="checkFinalAnswer(this)">
                                        @endif
                                        <label for="s-option">{{isset($arr_question['template5']['option2']) && $arr_question['template5']['option2']!='' ? $arr_question['template5']['option2'] : "N/A"}}</label>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-sm-6 col-md-4 col-lg-3">
                    <div class="specker-mic-section">
                        <button onclick="togglePlay()" id="hornButton">
                            ...
                            <audio id="horn" src="{{$question_audio_public_path.$arr_question['template5']['horn']}}"></audio>
                        </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template5']['duration']));
            $minutes     = date('i',strtotime($arr_question['template5']['duration']));
            $seconds     = date('s',strtotime($arr_question['template5']['duration']));
        }
        $actual_time = $arr_question['template5']['duration'];
...
<script type="text/javascript">
    ...
    var correct_answer                = parseInt("{{$arr_question['template5']['answer']}}");
</script>
...
<script type="text/javascript">
    ...
    {
        $('input[name="template5_checkbox"]').each(function(){
