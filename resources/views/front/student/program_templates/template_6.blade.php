...
@endphp
    <!--middle section-->
    <div class="middle-section temp-6-main-section">
        <div class="container">
            <div class="fill-in-the-blank-section">
                <div class="question-section">
                    <div class="question-txt-section">
                        <i class="fa fa-angle-double-right"></i> {{isset($arr_question['template6']['question']) && $arr_question['template6']['question']!='' ? ucwords($arr_question['template6']['question']) : "N/A" }}
                    </div>
                ...
                <div class="row">
                    <div class="col-sm-6 col-mg-6 col-lg-6">
                        ...
                        <div class="game-img-section">
                            @if(isset($arr_question['template6']['question_file']) && $arr_question['template6']['question_file']!='')
                                @php $result = get_image_public_path($arr_question['template6']['question_file'],'video');  @endphp
                                @if($result['status']=='success')
                                    <video src="{{$result['image_url']}}" alt="tamplate-five" controls="true" /></video>
                                @else
                                    <img src="{{$result['image_url']}}" alt="tamplate-five" /></video>
                                @endif
                            @endif
                        </div>
                    </div>
                                ...
                                <div class="radio-btns">  
                                    <div class="radio-btn">
                                        @if(isset($arr_question['is_answer']) && $arr_question['is_answer']=='yes')
                                            <input type="radio" id="f-option" name="template6_checkbox" value="1" disabled="true" @if($arr_question['template6']['answer']==1) checked="true" @endif>
                                        @else
                                            <input type="radio" id="f-option" name="template6_checkbox" value="1" onclick="checkFinalAnswer(this)">
                                        @endif
                                        <label for="f-option">{{isset($arr_question['template6']['option1']) && $arr_question['template6']['option1']!='' ? $arr_question['template6']['option1'] : "N/A"}}</label>
                                    </div>
                                    <div class="radio-btn">
                                        @if(isset($arr_question['is_answer']) && $arr_question['is_answer']=='yes')
                                            <input type="radio" id="s-option" name="template6_checkbox" value="2" disabled="true" @if($arr_question['template6']['answer']==2) checked="true" @endif>
                                        @else
                                            <input type="radio" id="s-option" name="template6_checkbox" @if(isset($arr_question['is_delay']) && $arr_question['is_delay']=='yes') time-over-answer-section @endif" value="2" onclick="checkFinalAnswer(this)">
                                        @endif
                                        <label for="s-option">{{isset($arr_question['template6']['option2']) && $arr_question['template6']['option2']!='' ? $arr_question['template6']['option2'] : "N/A"}}</label>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-sm-6 col-md-4 col-lg-3">
                    <div class="specker-mic-section">
                        <button onclick="togglePlay()" id="hornButton">
                            ...
                            <audio id="horn" src="{{$question_audio_public_path.$arr_question['template6']['horn']}}"></audio>
                        </button>
    ...
    @php
        ...
        {
            $hours       = date('H',strtotime($arr_question['template6']['duration']));
            $minutes     = date('i',strtotime($arr_question['template6']['duration']));
            $seconds     = date('s',strtotime($arr_question['template6']['duration']));
        }
        $actual_time = $arr_question['template6']['duration'];
...
<script type="text/javascript">
    ...
    var correct_answer                = parseInt("{{$arr_question['template6']['answer']}}");
</script>
...
<script type="text/javascript">
    ...
    {
        $('input[name="template6_checkbox"]').each(function(){
            if($(this).val()==correct_answer){
                $(this).parent().addClass('time-over-answer-section');   
            }
        });
    }
    function timeisUp()
