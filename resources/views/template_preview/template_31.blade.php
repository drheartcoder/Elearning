...
@php
    $uniqid = uniqid();
    $str_options = $arr_question['answer'];
    $arr_options = explode(',',$str_options);
    $question_string = $arr_question['question_1'];
    $maxlength   = 0;
    $question    = '';
@endphp
@if(isset($arr_options) && count($arr_options)>0)
    @foreach($arr_options as $key => $val)
        ...
        @endphp
            ...
            @php
            $replace    = '<input class="actual-ans" type="text" name="question" value="'.strtolower($val).'" disabled="true" />';
            @endphp
        @php
