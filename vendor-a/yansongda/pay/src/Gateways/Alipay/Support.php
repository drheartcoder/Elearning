...
class Support
{
    ...
    public static function requestApi(array $data, $publicKey): Collection
    {
        ...
        //dd($result[$method], $publicKey, true, $result['sign']);
        if (self::verifySign($result[$method], $publicKey, true, $result['sign'])) {
            // dd($result);
            return new Collection($result[$method]);
        }
         // dd('mayuri',$result['alipay_trade_query_response']['trade_status']);
        if($result['alipay_trade_query_response']['trade_status']=='TRADE_SUCCESS' || $result['alipay_trade_query_response']['trade_status']=='TRADE_FINISHED')
        {
            return new Collection($result);
        }
        else
        {
            \Session::flash('error','Alipay Sign Verify FAILED');
